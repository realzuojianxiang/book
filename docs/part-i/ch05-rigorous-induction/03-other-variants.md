...
*Proof.* Let P(n) be a variable proposition. Let M $\in$Z be arbitrary and fixed.
</div>
Let S = {z $\in$Z | z $\geq$M}. Suppose that (1) P(M) holds True, and (2) $\forall$k $\in$S. P(k) =$\Rightarrow$P(k + 1) holds True Our goal is to prove that $\forall$n $\in$S. P(n) holds True. Define the proposition Q(n) by setting Q(n) ⇐$\Rightarrow$P(n + M −1) Notice that, by algebraically manipulating the inequality, we have n $\geq$1 ⇐$\Rightarrow$n + M −1 $\geq$M This means that our goal now is to prove that $\forall$n $\in$N. Q(n) holds True. (Doing so will prove that $\forall$n $\in$S. P(n).) We will prove this by induction on n. BC: We know P(M) holds, by assumption. Notice that n + M −1 = M ⇐$\Rightarrow$ n = 1. This means Q(1) holds. IH: Let k $\in$N be arbitrary and fixed. Suppose Q(k) holds. IS: Since Q(k) holds, we know P(k + M −1) holds. Also, since k $\in$N, we know k $\geq$1. Thus, k + M −1 $\geq$M. Thus, By condition (2) that we assumed, we can deduce that P((k+M −1)+1) holds, i.e. that P(k + M) holds. This tells us that Q(k + 1) holds. By PMI, we deduce that $\forall$n $\in$N. Q(n) holds. Then, by the definition of Q(n), this tells us that $\forall$n $\in$S. P(n) holds.
...
Claim:
2n > n2 ⇐$\Rightarrow$n $\in${0, 1} $\cup${z $\in$N | z $\geq$5} That is, the only integers z that satisfy 2z > z2 are z = 0, 1, 5, 6, 7 . . .. (We will leave it to you to play around and figure out how we might have come up with such a claim. Typically, as you will see in this section's exercises, such an inequality might be presented along with the question, "For which values of n does this hold?" In that case, you would have to do some scratch work to identify your claim before starting an induction proof.)
<div class="def-proof" markdown>
*Proof.* Let P(n) be the proposition " 2n > n2 ".
</div>
First, observe the following cases: 20 > 02 ⇐$\Rightarrow$1 > 0 so P(0) is True 21 > 12 ⇐$\Rightarrow$2 > 1 so P(1) is True 22 > 22 ⇐$\Rightarrow$4 > 4 so P(2) is False 23 > 32 ⇐$\Rightarrow$8 > 9 so P(3) is False 24 > 42 ⇐$\Rightarrow$16 > 16 so P(4) is False Notice that whenever z $\leq$−1, we have 2z < 1 and z2 $\geq$1, so 2z ̸> z2. Thus, P(n) is False for every n that satisfies n $\leq$−1. Next, define S to be the set S = {z $\in$N | z $\geq$5}. We will prove $\forall$n $\in$S. P(n) holds by induction on n. BC: Observe that P(5) holds because 25 = 32 and 52 = 25 and 32 > 25. IH: Let k $\in$N be arbitrary and fixed. Suppose that P(k) holds. IS: Since k $\in$S, we know k $\geq$5 and so k > 4. Thus, k −1 > 3 and so (k −1)2 > 9; certainly, then, (k −1)2 > 2.


> 🇨🇳 TODO: 待翻译


Observe the following chain of manipulations of this inequality: (k −1)2 > 2 =$\Rightarrow$(k −1)2 −2 > 0 =$\Rightarrow$k2 −2k −1 > 0 =$\Rightarrow$k2 > 2k + 1 =$\Rightarrow$2k2 > k2 + 2k + 1 =$\Rightarrow$2k2 > (k + 1)2 Since we observed the first inequality holds, we can deduce that the final inequality above also holds. (Note: In case you didn't realize, this chain of reasoning is a solution to practice exercise 2 from Section 4.9.9! To work this out, we did some scratch work, starting with the desired inequality at the bottom and "working backwards" until we found something obviously True. In writing it up here, we started with that obvious fact and worked down to the desired conclusion.) By the IH P(k), we know k2 < 2k. This tells us 2k2 < 2 $\cdot$ 2k = 2k+1 Applying the transitivity property of inequalities, we can deduce that (k + 1)2 < 2k2 < 2k+1 and so P(k + 1) holds. By PMI, $\forall$n $\in$S. P(n) holds. Overall, we have considered every z $\in$Z. We observed that P(z) fails for z $\leq$−1, that it holds for z = 0, 1, that it fails for z = 2, 3, 4, and that it holds for z $\geq$5. Together, these observations prove the claim. Phew! There was actually a lot going on in that proof. Did you notice that the claim was phrased as an if and only if, so that we had to consider all the integers in our proof? That was tricky, but we did it!
...
O = {n $\in$N | $\exists$m $\in$N $\cup${0}. n = 2m + 1} Let P(n) be the proposition "n2 is 1 more than a multiple of 8". Then $\forall$n $\in$O. P(n)
<div class="def-proof" markdown>
*Proof.* Let P(n) be defined as in the claim above. We will prove $\forall$n $\in$O. P(n)
</div>
by induction on n. BC: Observe that 12 = 1 and 1 = 0 $\cdot$ 8 + 1 (i.e. 1 is a multiple of 8 plus 1). Thus, P(1) holds. IH: Let k $\in$O be arbitrary and fixed. Suppose P(k) holds. IS: Our goal now is to deduce P(k +2) holds. (This is because k +2 is the next odd natural number, after k.) Since k + 2 is odd, by assumption, we know $\exists$m $\in$N $\cup${0}. k = 2m + 1. Let such an m be given. By the IH, we know $\exists$ℓ$\in$N. k2 = 8ℓ+ 1. Let such an ℓbe given. Now, we take these observations and use them to see that (k + 2)2 = k2 + 4k + 4 = (8ℓ+ 1) + 4(2m + 1) + 4 = 8ℓ+ 8m + 8 + 1 = 8(ℓ+ m) + 1 Since ℓ, m $\in$Z, we know ℓ+ m $\in$Z, as well. Thus, (k + 2)2 is one more than a multiple of 8. Therefore, P(k + 2) holds.
...
 1 + (−1)n  (a) Let O be the set of odd natural numbers. Prove that $\forall$n $\in$O. P(n) by induction. (b) Let E be the set of even natural numbers. Prove that $\forall$n $\in$E. P(n) by induction. (6) Consider the following claim: n X k=1 (−1)k−1k2 = (−1)k−1 n X k=1 k That is, we claim that 12 −22 + 32 −42 + $\cdot$  $\cdot$  $\cdot$ + (−1)n−1n2 = (−1)n−1(1 + 2 + 3 + $\cdot$  $\cdot$  $\cdot$ + 9) holds for every n $\in$N. (a) Prove that the above formula holds for n = 1 and n = 2. (b) Prove that whenever the formula holds for k, it also holds for k + 2. (c) Explain intuitively why (a) and (b) prove the claim.
