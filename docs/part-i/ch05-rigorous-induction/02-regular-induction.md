...
" 1 + 3 + 5 + · · · + 2n −1 = n X i=1 (2i −1) = n2 " We will prove that ∀n ∈N. P(n) by induction on n. BC: Consider n = 1. Notice that
...
IH: Let k ∈N be arbitrary and fixed. Suppose P(k) holds. This means n X i=1 (2k −1) = n2 IS: Consider k + 1. We can write k+1 X i=1 (2i −1) = 2(k + 1) −1 + k X i=1 (2i −1) = 2k + 1 + k X i=1 (2i −1) by separating out the (k + 1)-th term of the summation. We now use the IH to replace the summation on the right-hand side and deduce that k+1 X i=1 (2i −1) = 2k + 1 + k2
Factoring then tells us
k+1 X i=1 (2i −1) = (k + 1)2 and therefore P(k + 1) holds. By the PMI, we conclude that ∀n ∈N. P(n). Here's another good induction proof of a useful fact about geometric series.
Example 5.2.6. Geometric series formula:
Claim: For every q ∈R −{0, 1} and for every n ∈N, the following formula
holds: n−1 X i=0 qi = 1 + q + q2 + · · · + qn−1 = qn −1 q −1
<div class="def-proof" markdown>
*Proof.* Let q ∈R −{0, 1} be arbitrary and fixed. Define P(n) to be the propo-
</div>
sition " n−1 X i=0 qi = qn −1 q −1 " We will prove ∀n ∈N. P(n) by induction on n. BC: Consider n = 1. Observe that n−1 X i=0 qi =


> 🇨🇳 TODO: 待翻译


X i=0 qi = q0 = 1 since q ̸= 0. Also, observe that qn −1 q −1 = q −1 q −1 = 1


> 🇨🇳 TODO: 待翻译


since q ̸= 1. Thus, P(1) holds. IH: Let k ∈N be arbitrary and fixed and suppose P(k) holds. This means k−1 X i=0 qi = qk −1 q −1 IS: WWTS P(k + 1) holds. (Remember, WWTS means "We want to show".) That is, WWTS k X i=0 qi = qk+1 −1 q −1 noticing that (k + 1) −1 = k. Observe that we can algebraically simplify and use our assumptions to write k X i=0 qi = k−1 X i=0 qi ! + qk summation notation = qk −1 q −1 + qk invoking IH = qk −1 + qk(q −1) q −1 common denominator = qk −1 + qk+1 −qk q −1 = qk+1 −1 q −1 algebra This shows that P(k + 1) holds, as well. By the PMI, ∀n ∈N. P(n) holds. Follow-up question: Why did we need q /∈{0, 1} in the claim? What happens when q = 0? Where does this proof break down? Does the formula still holds? If so, prove it. If not, can you fix it? Try answering the same questions for q = 1, as well.
...
2 holds for every n ∈N. (2) Prove that the square of every odd natural number is one more than a multiple of 8. That is, prove that (2n + 1)2 −1 is a multiple of 8 for every n ∈N. (3) Consider this claim: 7n −4n is a multiple of 3, for every n ∈N. Rewrite this claim using logical symbolic notation. Then, prove it by induction. (4) Recall that the Fibonacci Numbers are defined by f0 = 0 and f1 = 1 and ∀n ∈N −{1}. fn = fn−1 + fn−2 Prove the following claims hold for every n ∈N, by induction on n: (a) n X i=1 fi = fn+2 −1 (b) n X i=1 f2i−1 = f2n (c) f4n is a multiple of 3 (d) Challenge 1: (n is a multiple of 3) =⇒(fn is even) (e) Challenge 2: (n is not a multiple of 3) =⇒(fn is odd)


> 🇨🇳 TODO: 待翻译


## 5.3 Other Variants of Induction Now that we're really comfortable with how induction works and have seen many examples, we can show you two modifications of this method. The idea is that there is "nothing special" about using induction to prove a statement holds for every n ∈N. Don't get us wrong; there is a lot that's special about N! What we mean is that it's possible to use induction to prove that a statement holds for every n ∈S, where S might be some other kind of set. We will describe these sets for you in the following discussions and examples.
...
sition. Let M ∈Z be arbitrary and fixed. Let S = {z ∈Z | z ≥M}. Suppose that (1) P(M) holds True, and (2) ∀k ∈S. P(k) =⇒P(k + 1) holds True Then ∀n ∈S. P(n) holds True. This theorem says exactly what we were discussing: if we want to prove a proposition holds for every value greater than or equal to some specific value (M, in the theorem statement), then we can just start inducting from that value.
