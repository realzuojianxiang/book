...
(3) Give an example of a set X that satisfies N $\in$X but Z /$\in$X. (4) Give an example of a set with 100 elements. (5) Give an example of a sets A, B, C such that A $\in$B and B $\in$C but A /$\in$C. (6) Write a definition of the set of odd integers using set-builder notation. (7) Write a definition of the set of integers that are not natural numbers using set-builder notation. (8) Consider the following sets: A =
 ...
 x $\in$R | x2 −3x + 2 $\geq$0


> 🇨🇳 (3)-(7) 练习题。


B = {y $\in$R | y $\leq$1 or y $\geq$2} Explain why A = B. (9) Consider the following sets: C =
