...
{0} $\subseteq$ \ n$\in$N An This requires us to show that 0 $\in$An for every n $\in$N. Let n $\in$N be arbitrary and fixed. Notice that the inequality 0 $\le$0 < 1 n does, indeed hold. (Note: You might be worried because "in the limit" 0 is not less than every fraction
...
n "all at once", but that is not the point! Think of it this way: Is 0 $\in$A1? Yes, 0 $\le$0 < 1. Is 0 $\in$A2? Yes, 0 $\le$0 < 1 2. Is 0 $\in$A3? Yes, 0 $\le$0 < 1 3. And so on. The inequality holds for every n $\in$N individually, so 0 is an element of every such set. If you weren't worried about this, never mind! Move right along!) Thus, 0 $\in$An for every n $\in$N, and so 0 $\in$ \ n$\in$N An, by the definition of "T". This shows that {0} $\subseteq$ \ n$\in$N An.
...
. Prove that \ n$\in$N Sn = {0} (9) Let I = {x $\in$R | 0 < x < 1}. For every x $\in$I, define Sx = {y $\in$R | x < y < x + 1}. Prove that [ x$\in$I Sx = {z $\in$R | 0 < z < 2} (10) For every n $\in$N, define the sets An and Bn by An =

x $\in$R | 0 $\le$x < n −1 n  Bn =

y $\in$R | −1 n < y < 1  Prove the following set equality by a double-containment argument: [ n$\in$N An = \ n$\in$N Bn
