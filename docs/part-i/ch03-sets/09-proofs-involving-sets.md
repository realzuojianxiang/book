...
n "all at once", but that is not the point! Think of it this way: Is 0 $\in A$1? Yes, 0 $\le$0 < 1. Is 0 $\in A$2? Yes, 0 $\le$0 < 1 2. Is 0 $\in A$3? Yes, 0 $\le$0 < 1 3. And so on. The inequality holds for every n $\in \mathbb{N}$ individually, so 0 is an element of every such set. If you weren't worried about this, never mind! Move right along!) Thus, 0 $\in A$n for every n $\in \mathbb{N}$, and so 0 $\in$ \ n$\in \mathbb{N}$ An, by the definition of "T". This shows that {0} $\subseteq$ \ n$\in \mathbb{N}$ An.
