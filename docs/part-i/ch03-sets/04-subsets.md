...
Examples
A set is a subset of another set if and only if every element of the first one is an element of the second one. For instance, this means that the following claims all hold: {142, 857} $\subseteq$N n√ 3, −π, 8.2 o $\subseteq$R
...
$\subseteq$Z Do you see why these are True? For a subset relationship to fail, then, we must be able to find an element of the first set that is not an element of the second set. For instance, this means that the following claims all hold: {142, −857} ̸$\subseteq$N n√ 3, −π, 8.2 o ̸$\subseteq$Q
