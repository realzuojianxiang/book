...
equality holds? {$x \in \mathbb{Z}$ | $x \geq 1$} = N We just need to see that the $\subseteq$ and $\supseteq$ relationship applies between the two sides. First, is it true that every integer that is at least 1 is a natural number? Yes! This explains why {$x \in \mathbb{Z}$ | $x \geq 1$} $\subseteq \mathbb{N}$ Second, is it true that every natural number is a positive integer that is at least 1? Yes! This explains why {$x \in \mathbb{Z}$ | $x \geq 1$} $\supseteq \mathbb{N}$ Together, this shows that the equality stated above is correct.
