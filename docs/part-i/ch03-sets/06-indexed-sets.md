...
Ai = {i −2, i −1, i, i + 1, i + 2} Let's identify and write out the elements of the following sets: [ i$\in I$ Ai and \ i$\in I$ Ai Notice that we can write out the elements of each Ai set, as follows: A1 = {−1, 0, 1, 2, 3} A2 = {0, 1, 2, 3, 4} A3 = {1, 2, 3, 4, 5} Thus, [ i$\in I$ Ai = A1 $\cup A$2 $\cup A$3 = {−1, 0, 1, 2, 3, 4, 5} and \ i$\in I$ Ai = A1 $\cap A$2 $\cap A$3 = {1, 2, 3} Now, consider J = {−1, 0, 1}, with Aj defined in the same way as before. Let's identify the elements of the sets [ j$\in J$ Aj and \ j$\in J$ Aj Writing out the elements of each set, we can determine that [ j$\in J$ Aj = A−1 $\cup A$0 $\cup A$1 = {−3, −2, −1, 0, 1, 2, 3} and \ j$\in J$ Aj = A−1 $\cap A$0 $\cap A$1 = {−1, 0, 1} Try answering the same questions with different index sets. For instance, consider K = {1, 2, 3, 4, 5} or L = {−3, −2, −1, 0, 1, 2, 3}.
...
(2) Consider the set R. For every z $\in \mathbb{Z}$, define the set Sz by Sz = {r $\in \mathbb{R}$ | z $\leq r$ < z + 1} We claim {. . . , S−2, S−1, S0, S1, S2, . . . } is a partition of R. Can you see why? Try to write out the conditions required for this collection of sets to be a partition, and see if you can understand why they hold. Specifically, remember that we need these sets to be pairwise disjoint. This means that any two sets must be disjoint. Notice that this is quite different from requiring the the intersection of all the sets together to be empty. For instance, consider the collection of sets  {1, 2}, {2, 3}, {3, 4}
