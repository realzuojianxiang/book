...
3.5.2, we can say
S1 $\cup S$2 = {1, 2, 3, 4, 5, 7} S1 $\cup S$3 = {1, 2, 3, 4, 5, 7} S2 $\cup S$3 = {1, 2, 3, 4, 7} Also, since each of these unions are sets themselves, we could find their union with another set. For example, (S1 $\cup S$2) $\cup S$3 = {1, 2, 3, 4, 5, 7} $\cup${2, 4, 7} = {1, 2, 3, 4, 5, 7} Unions and Subsets Notice that A $\subseteq$(A $\cup B$) and B $\subseteq$(A $\cup B$), no matter what A and B are. Let's prove that!
...
> 🇨🇳 **3.5.5 问题与练习** (1) 并集和交集的区别？(2) 两个集合不相交意味着什么？(3) $\mathbb{Z} \cap \mathbb{N}$？$\mathbb{Z} \cup \mathbb{N}$？$\mathbb{Z} - \mathbb{N}$？(4) $A - B = B - A$ 可能成立吗？(5) $[3]$ 在 $\mathbb{N}$ 和 $\mathbb{Z}$ 和 $\mathbb{R}$ 语境中分别是什么？(6) $(A \cap B) \cap C = A \cap (B \cap C)$ 总是成立吗？(7) "7 − 5" 和 "[7] − [5]" 有什么不同？(8) $A - x$ 写法正确吗？(9) $(\mathbb{Z} - \mathbb{N}) \cup \mathbb{R}$ 是什么？
