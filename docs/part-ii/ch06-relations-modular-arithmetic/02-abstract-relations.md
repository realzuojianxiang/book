...
• We say $R$ is reflexive if $\\f\forall x \\in A.\\, (x, x) \\in R$. That is, every element is related to itself. • We say $R$ is symmetric if $\\f\forall x, y \\in A.\\, (x, y) \\in R \$\Rightarrow$ (y, x) \\in R$. That is, the order of the comparison doesn't matter.
...
>
> • 若 $\f\forall x \in A. (x, x) \\in R$，则称 R 是**自反的**（Reflexive），即每个元素与自身相关。
> • 若 $\f\forall x, y \in A. (x, y) \in \mathbb{R} $\Rightarrow$ (y, x) \\in R$，则称 R 是**对称的**（Symmetric），即比较的顺序无关紧要。


• We say $R$ is transitive if $\\f\forall x, y, z \\in A.\\, [(x, y) \\in R \\wedge (y, z) \\in R] \$\Rightarrow$ (x, z) \\in R$ That is, relationships can transition through a middle-man. • We say $R$ is anti-symmetric if $\\f\forall x, y \\in A.\\, [(x, y) \\in R \\wedge (y, x) \\in R] \$\Rightarrow$ x = y$ That is, two different elements can be related in at most one way, or not at all. To see why this is the same statement, let's look at the contrapositive of the conditional statement in the line above: $\\f\forall x, y \\in A.\\, x \\neq y \$\Rightarrow$ [(x, y) \\notin R \\vee (y, x) \\notin R]$
...
> 🇨🇳 • 若 $\\f\forall x, y, z \\in A.\\, [(x, y) \\in R \\wedge (y, z) \\in R] \$\Rightarrow$ (x, z) \\in R$，则称 R 是**传递的**（Transitive），即关系可以通过"中间人"传递。
>
> • 若 $\\f\forall x, y \\in A.\\, [(x, y) \\in R \\wedge (y, x) \\in R] \$\Rightarrow$ x = y$，则称 R 是**反对称的**（Anti-symmetric），即两个不同元素至多以一种方式相关（或完全不相关）。等价地，观察条件语句的逆否命题：$\\f\forall x, y \\in A.\\, x \\neq y \$\Rightarrow$ [(x, y) \\notin R \\vee (y, x) \\notin R]$。
>
...
• If $R$ is reflexive, transitive, and anti-symmetric, then we say R is a partial order on A. • If $R$ is reflexive, transitive, and anti-symmetric and, in addition, it satisfies $\\f\forall x, y \\in A.\\, (x, y) \\in R \\vee (y, x) \\in R$ then we say R is a total order on A. (That is, a total order is a partial order such that every two elements of the set are comparable one way or the other.) This definition tells us what partial and total orders are. The next definition just gives us some useful shorthand for referring to partial and total orders on sets.
<div class="def-definition" markdown>
...
> • 若 R 是偏序且满足 $\f\forall x, y \in A. (x, y) \in \mathbb{R} \vee (y, x) \\in R$（即任意两个元素都可比较），则称 R 为 A 上的**全序**（Total Order）。
>
