...
>
> **任何有序对的集合都是关系** **注 6.2.8.** 给定集合 A、B，任何子集 $R \subseteq A \times B$ 都定义了一个关系，即使很难（甚至不可能）找到刻画它的性质。例如，若 A = {1, 3, 5}，B = {⋆, ♥}，则 $R = \{(1, \text{⋆}), (5, \text{♥})\}$ 就定义了一个关系。为什么 1 与 ⋆ 相关？为什么 3 不与任何元素相关？谁知道呢——它就是一个有序对的集合！在数学上完全合法。
