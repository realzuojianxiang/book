...
) such solutions. (3) How many solutions satisfy x1, x2 $\geq$5 and x3, x4, x5 \geq2? Using a method exactly like the last one, we see that we're trying to solve y1 + y2 + y3 + y4 + y5 = 9 where yi = xi -5 for i = 1, 2, and yi = xi -2 for i = 3, 4, 5. The right-hand side has been changed to 25 -5 -5 -2 -2 -2 = 9. By the formula, we know there are (9 + (5 -1) 5 -1 ) such solutions. (4) How many solutions satisfy x2 \leq5? We can do this in one of two ways. First, let's take the total number of solutions (found in the first part of this example) and remove the number of solutions that fail this desired condition. That is, let's take the number of solutions where x2 $\geq$6, which is ((25 -6) + (5 -1) 5 -1 ) = (23
