...
$\forall x \in \mathbb{R}$. f(x) = ⌊x⌋ (Recall that ⌊x⌋is the floor of x: it is the largest integer $z \in \mathbb{Z}$ that satisfies z \leq x. Let g : $\mathbb{Z}$ $\to \mathbb{N}$ be the function defined by \forall $z \in \mathbb{Z}$. g(z) = ( -z if z < 0 z + 1 if z \geq0 Let's find g \circ f. Notice that whenever $x \in \mathbb{R}$ satisfies x < 0, we will have ⌊x⌋x < 0, as well. Similarly, whenever $x \in \mathbb{R}$ satisfies x \geq0, we will have ⌊x⌋\geq0. This tells us that the composition g \circ f will also be a piece-wise function: \forall $x \in \mathbb{R}$. (g \circ f)(x) = ( -⌊x⌋ if x < 0 ⌊x⌋+ 1 if x $\geq 0$
