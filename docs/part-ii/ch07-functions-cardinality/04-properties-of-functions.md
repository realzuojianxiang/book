...
Example 7.4.2. Consider p : $N \times N \to \mathbb{N}$ defined by p(a, b) = ab. Is p surjective?
...
Notice that (1, n) $\in N \times N and p(1, n) = 1 \cdot$ n = n. Since n was arbitrary, this shows p is surjective.
...
Example 7.4.4. Let d : $N \times N \to \mathbb{Z}$ be the function defined by
$\forall(a, b) \in N \times N. d(a, b) = a$ −b
...
x+1, we deduce that 1 > x x+1. Certainly, then, g(x) = x x+1 \neq 1. • Suppose x < −1. This means x + 1 < 0, and so
...
x+1 and switching the sign, we deduce that 1 < x x+1. Certainly, then, g(x) = x x+1 \neq 1. In either case g(x) \neq 1. These cases cover all possibilities because x $\in \mathbb{R}$−{−1} was arbitrary (and we need not consider x = −1). This shows 1 $\notin I$mg(R −{−1}) so g is not a surjection.


> 🇨🇳 **证明非单射** 只需找到两个不同输入产生相同输出：定义 $x$ 和 $y$，验证 $x \neq y$ 且 $f(x) = f(y)$。


Notice that this first proof proves an interesting qualitative observation about the graph: that the function lies above the horizonatal asymptote to the left of x = −1 and above the asymptote to the right of x = −1. Proof 2 (Contradiction). AFSOC that g is surjective. This means $\forall y \in \mathbb{R}. y \in I$mg(R −{−1}) In particular, then, we know 1 $\in I$mg(R −{−1}), so $\exists x$ $\in \mathbb{R}$ −{−$1}. g(x) = 1. Let such an x be given. This means g(x) = x x+1 = 1. Multiplying both sides, we find x = x + 1. Subtracting, we find 0 = 1, clearly a contradiction \times\times\times\times Therefore, 1 / \in I$mg(R −{−1}), so g is not a surjection. Notice that this second proof does prove that g is not a surjection, but it doesn't add any other information about how the function behaves (like the previous proof did). Let's move on from surjections and talk about a closley related property of functions.
...
Example 7.4.7. Consider p : $N \times N \to \mathbb{N}$ defined by p(a, b) = ab. Is p injective?
...
Example 7.4.9. Let d : $N \times N \to \mathbb{Z}$ be the function defined by d(a, b) = a −b.
Determine whether d is an injection and prove your claim. It turns out d is not an injection! Notice that a −b = (a + 1) −(b + 1). We can use this to find a counterexample: Consider the pairs (2, 1) $\in N \times N and (3, 2) \in N \times N. Notice that d(2, 1) = 1 and d(3, 2) = 1. Since (2, 1)$ \neq (3, 2) and yet d(2, 1) = d(3, 2), we conclude that d is not an injection.
...
Example 7.4.12.
(a) Let p : $N \times N \to \mathbb{N}$ be the function defined by p(a, b) = ab. We proved that p is surjective but not injective, so it is not a bijection. (b) Let d : $N \times N \to \mathbb{Z}$ be the function defined by d(a, b) = a −b. We proved that d is surjective but not injective, so it is not a bijection. (c) Let g : R −{−1} $\to \mathbb{R}$ be the function defined by $\forall x$ $\in \mathbb{R}$. g(x) = x 1 + x We proved that g is not surjective. (Specifically, we showed 1 $\notin I$mg(R − {−1}).) We will ask you in this section's exercises to prove that g is an injection, though. Together this means g is not a bijection. However, consider defining h : R −{−1} $\to \mathbb{R}$ −{1} by the same "rule" as g, i.e. $\forall x$ $\in \mathbb{R}$ −{1}. h(x) = x 1+x. We asked you to prove in the exercises of Section 7.3.5 that this function satisfies Imh(R −{−1}) = R −{1}. This shows that h is a surjection. Furthermore, we will ask you to prove in this section's exercises that a function defined in this way---by taking an injection, using the same "rule", and redefining the codomain to be the image---produces a bijection. Together, all of this proves that h is a bijection from R−{−1} to R−{1}.
...
natural numbers; that is, E = {e $\in \mathbb{N}$ | $\exists k$ $\in \mathbb{N}$. e = 2k} Define the function d : N $\to E$ by d(n) = 2n. We claim d is a bijection.
<div class="def-proof" markdown>
...
Examples
Example 7.5.2. Let C : R $\to \mathbb{R}$ be defined by
$\forall x \in \mathbb{R}$. C(x) = x −273.15 Let F : R $\to \mathbb{R} be defined by \forall x \in \mathbb{R}$. F(x) = 9 5x + 32 The function C converts a temperature from degrees Kelvin to degrees Celsisus. The function F converts from degrees Celsius to degrees Fahrenheit. Then the function F ◦C converts from degrees Kelvin to degrees Fahrenheit


> 🇨🇳 **有限函数的计数** 从 $[m]$ 到 $[n]$ 的函数数为 $n^m$。单射函数数（$m \leq n$ 时）为 $P(n,m) = \frac{n!}{(n-m)!}$。双射函数数（$m=n$ 时）为 $n!$。


directly. We can compose the "rules" for the functions and find a formula for this direct conversion: $\forall x \in \mathbb{R}$. (F ◦C)(x) = F(C(x)) = F(x −273.15) = 9 5 $\cdot$  x −273.15) + 32 = 9 5x −459.67
Example 7.5.3. Let f : R $\to \mathbb{Z}$ be the function defined by
$\forall x \in \mathbb{R}$. f(x) = ⌊x⌋ (Recall that ⌊x⌋is the floor of x: it is the largest integer z $\in \mathbb{Z}$ that satisfies z $\leq x$. Let g : Z $\to \mathbb{N}$ be the function defined by $\forall z$ $\in \mathbb{Z}$. g(z) = ( −z if z < 0 z + 1 if z $\geq$0 Let's find g ◦f. Notice that whenever x $\in \mathbb{R}$ satisfies x < 0, we will have ⌊x⌋x < 0, as well. Similarly, whenever x $\in \mathbb{R}$ satisfies x $\geq$0, we will have ⌊x⌋$\geq$0. This tells us that the composition g ◦f will also be a piece-wise function: $\forall x$ $\in \mathbb{R}$. (g ◦f)(x) = ( −⌊x⌋ if x < 0 ⌊x⌋+ 1 if x $\geq$0
Questions: Is this function injective? Surjective? Try to prove your claims!
Example 7.5.4. Define f : N $\to \mathbb{N}$ and g : N $\to \mathbb{N}$ and h : N $\to \mathbb{N}$ by
$\forall n \in \mathbb{N}$. f(n) = n + 3 $\forall n \in \mathbb{N}$. g(n) = n2 $\forall n \in \mathbb{N}$. h(n) = 2n −1 (Question: Are you sure these are well-defined functions? Why?) We can find "rules" for the compositions g ◦f and h ◦f: $\forall n \in \mathbb{N}$. (g ◦f)(n) = g(f(n)) = g(n + 3) = (n + 3)2 = n2 + 6n + 9 $\forall n \in \mathbb{N}$. (h ◦g)(n) = h(g(n)) = h(n2) = 2n2 −1 We can then use these to find a rule for a further composition, like h ◦(g ◦f): $\forall n \in \mathbb{N}$.  h ◦(g ◦f)  (n) = h  (g ◦f)(n)  = h  n2 + 6n + 9  = 2(n2 + 6n + 9) −1 = 2n2 + 12n + 17 Likewise, we can use these to find a rule for (h ◦g) ◦f: $\forall n \in \mathbb{N}$.  (h ◦g) ◦f  (n) =  h ◦g)(f(n)) = (h ◦g)(n + 3) = 2(n + 3)2 −1 = 2(n2 + 6n + 9) −1 = 2n2 + 12n + 17
...
(Notice that this doesn't assume any properties of g; it doesn't even have to be injective, necessarily! As an exercise, try to find an example of functions f : A $\to B$ and g : B $\to \mathbb{C}$ such that g ◦f is injective and g is injective, and also an example where g ◦f is injective but g is not injective.)
<div class="def-proof" markdown>
...
Define A = {1, 2} and B = {♥, ♦} and C = {$\star$}. Define f by setting f(1) = ♥and f(2) = ♦. Notice f is injective because f(1) \neq f(2). Define g by setting g(1) = g(2) = $\star$. Notice g ◦f is defined by (g ◦f)(1) = $\star$and (g ◦f)(2) = $\star$. This shows g ◦f is not injective, because (g ◦f)(1) = (g ◦f)(2) but 1 \neq 2.
...
Define h : A $\to \mathbb{C}$ to be h = g ◦f. Then h is also bijective.
<div class="def-proof" markdown>
*Proof.* Left for the reader as Problem 7.8.9.
</div>
Proposition 7.5.13. Let f : A $\to B$ and g : B $\to \mathbb{C}$ be bijective functions.
Define h : A $\to \mathbb{C}$ to be h = g ◦f. Then h is invertible and h−1 = f −1 ◦g−1.
<div class="def-proof" markdown>
