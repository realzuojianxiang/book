...
if n is odd Let's show F = f −1. Let z $\in \mathbb{Z}$ be given. We have two cases. • Suppose z \geq1. Then f(z) = 2z −1. Notice that 2z −1 $\in \mathbb{N}$ and 2z −1 is odd. This means (F ◦f)(z) = F(f(z)) = F(2z −1) = (2z −1) + 1
...
• Suppose z $\leq$0. Then f(z) = −2z+2. Notice that −2z \geq0 so −2z+2 \geq2 so −2z + 2 $\in \mathbb{N}$. Also, −2z + 2 is even. This means (F ◦f)(z) = F(f(z)) = F(−2z + 2) = −−2z + 2


> 🇨🇳 **无限集的直觉** 无限集可以与自己的一部分等势——如 $|\mathbb{N}| = |2\mathbb{N}|$（偶数集）。这在有限集中不可能：有限集不能与自己的真子集等势。


+ 1 = −(−z + 1) + 1 = (z −1) + 1 = z In either case, (F ◦f)(z) = z. This shows F ◦f = IdZ. Next, let n $\in \mathbb{N}$. We have two cases. • Suppose n is even. Then F(n) = −n 2 + 1. Notice that n 2 \geq1 and so −n 2 \leq−1 + 1 = 0. This means (f ◦F)(n) = f(F(n)) = f  −n 2 + 1  = −2  −n 2 + 1  + 2 = 2n 2 −2  + 2 = n • Suppose n is odd. Then F(n) = n+1 2 . Notice that n + 1 $\geq$2 and so n+1
...
We claim this is a bijection. Since f, g are invertible, we claim that H : $N\timesN \to A \times B given by \forall$(k, ℓ) $\in \mathbb{N} \times \mathbb{N}. H(k,$ ℓ) =  f −1(k), g−1(ℓ)  satisfies H = h−1. To see why, notice that $\forall(x, y) \in A \times B. (H$ ◦h)(x, y) = H(h(x, y)) = H  f(x), g(y)  =  f −1(f(x)), g−1(g(y))  = (x, y) and \forall(k, ℓ) $\in \mathbb{N} \times \mathbb{N}. (h$ ◦H)(k, ℓ) = h(H(k, ℓ)) = h  f −1(k), g−1(ℓ)  =  f(f −1(k)), g(g−1(ℓ))  = (k, ℓ) so H ◦$h = IdA\timesB and h$ ◦$H = IdN\timesN. This shows H = h$−1. Therfore, h is a bijection, and so |$A \times B$| = |$N \times \mathbb{N}$| = |N|. By applying induction to the two previous results, we can prove the following:
<div class="def-theorem" markdown>
