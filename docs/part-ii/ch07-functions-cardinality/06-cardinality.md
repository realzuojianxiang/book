...
Before moving on to another example, let's show you one more way of thinking about why |$\mathbb{N} \times \mathbb{N}$| = |N|$. This will be an intuitive explanation, more like a description of how to define a bijection between the sets without actually making the definition. However, it's a common argument and is well worth seeing. The idea is to think of $\mathbb{N} \times \mathbb{N}$ as a lattice of points, like so:
...
k. (e) Let e : P(N) $\to P$(Z) be defined by taking in a set of natural numbers and outputting the set of all integer multiples of the least element of that set. Problem 7.8.2. Consider the sets R3 = {(x, y, z) | x, y, z $\in \mathbb{R}$} and $\mathbb{R}^2$ = {(a, b) | a, b $\in \mathbb{R}$}. Consider the function f : R3 $\to \mathbb{R}$2 defined by f(x, y, z) = (xz, yz). Is f injective? Surjective? Prove your claims.
