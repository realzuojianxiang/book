...
• Let $f: A \to B$ be a function. Let $Z \subseteq B$. The pre-image of $Z$ under the function $f$ is $\text{PreIm}_f(Z) = \{a \in A \mid f(a) \in Z\}$. (Intuitively, this is the set of all "inputs" whose output "lands" in $Z$.)
...
• Let $f: A \to B$ and $g: B \to C$ and $h: C \to D$ be functions. Then $(h \circ g) \circ f = h \circ (g \circ f)$. This means composition is associative.
...
-- Define $x = \ldots$ and define $y = \ldots$
...
Alternatively, -- AFSOC $f$ is bijective, so it has an inverse $f^{-1}$. Find a contradiction.

• For some $X \subseteq A$, find the image $\text{Im}_f(X)$:
-- Define a set $S$. Claim that $S = \text{Im}_f(X)$. (Note: Coming up with this definition is the hard part, and will involve a bunch of scratch work. There is no need to show this as part of your proof. Just start with the definition.)
...
-- Define a set $T$. Claim that $T = \text{PreIm}_f(Z)$. (Note: Coming up with this definition is the hard part, and will involve a bunch of scratch work. There is no need to show this as part of your proof. Just start with the definition.)
...
-- Define a function $F: B \to A$. (Note: Coming up with this definition is the hard part, and will involve a bunch of scratch work. There is no need to show this as part of your proof. Just start with the definition.)
