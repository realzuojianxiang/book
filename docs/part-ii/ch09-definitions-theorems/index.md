...
• To say "$S$ is a proper subset of $T$" we write $S \subset T$.
...
• Let $S$ be a set. The power set of $S$ is denoted by $\mathcal{P}(S)$ and is defined by $\mathcal{P}(S) = \{A \mid A \subseteq S\}$.
