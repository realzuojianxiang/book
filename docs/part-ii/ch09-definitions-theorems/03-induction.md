...
• Let $A, B$ be sets. A relation between $A$ and $B$ is a set of ordered pairs $R \subseteq A \times B$. Given elements $a \in A$ and $b \in B$, we say $a$ and $b$ are related if and only if $(a, b) \in R$. The set $A$ is called the domain and the set $B$ is called the codomain. The set $R$ is called the relation set. We say $R$ is a relation between $A$ and $B$. When $A = B$, we say $R$ is a relation on the set $A$.
